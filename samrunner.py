#this code to test with sam files directly
import pandas as pd
...
if len(sys.argv)<2:
    print("$ipython3 samrunner.py run_dir.pkl")
    sys.exit(0)
run_dir=sys.argv[1]
with open(run_dir, 'rb') as f:
   alignDict=pickle.load(f)
#print(alignDict)
...
if alignDict:
	...
	findCmd = 'find '+samDirectory+'no*.sam.gz'
	p0=subprocess.Popen(findCmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE, shell=True)
	samFileList=p0.stdout.read().decode('UTF-8').split('\n')
	samFileList=list(filter(None,samFileList))
	print(samDirectory)
	sam_count=0
	shrtFileList=[]
	print(samFileList)
...
#modify
		...
		#while cr:
		    for strnd in ['-','+']:
		        ...
		        vis_data.index=vis_data.QNAME.map(str)+'|'+vis_data.READ_NO.map(str)+'|'+vis_data.TOP_MAP.map(str)#+'|'+vis_data.TYPE.map(str)
